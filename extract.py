...
def main():
    ...
    for page_idx, payload in list_closed_pr_pages(http, args.owner, args.repo, per_page=args.per_page):
        ...
        for pr in payload:
            ...
            if m_at:
                ...
                if (since_dt is None or m_dt >= since_dt) and (until_dt is None or m_dt <= until_dt):
                    merged_prs.append(pr)
                    total_merged_in_window += 1

    # Debugging (make sure PRs being seen and correctly saved if within window)
    print(f"[STATS] Closed PRs seen: {total_closed}")
    print(f"[STATS] PRs with merged_at (any time): {total_merged_anytime}")
    print(f"[STATS] PRs merged in window: {total_merged_in_window}")

    # Enrich merged PRs with reviews + statuses + checks + branch protection (optional) 
    enriched: list[dict] = []
    for pr in merged_prs:
        pr_number = pr["number"]
        # Prefer merge_commit_sha if present, else use the head SHA
        head_sha = pr.get("merge_commit_sha") or pr["head"]["sha"]
        base_branch = pr["base"]["ref"]

        # Each call is wrapped so lack of permissions won't crash the run;
        # instead store an {"error": "..."} object for transparency.
        try:
            reviews = list_reviews(http, args.owner, args.repo, pr_number)
        except Exception as e:
            reviews = {"error": str(e)}

        try:
            combined_status = get_combined_status(http, args.owner, args.repo, head_sha)
        except Exception as e:
            combined_status = {"error": str(e)}

        try:
            check_runs = list_check_runs(http, args.owner, args.repo, head_sha)
        except Exception as e:
            check_runs = {"error": str(e)}

        try:
            required_status = get_required_status_contexts(http, args.owner, args.repo, base_branch)
        except Exception as e:
            required_status = {"error": str(e)}

        # Build one consolidated record per PR that transform.py utilises
        enriched.append({
            "pr": pr,
            "reviews": reviews,
    ...
    # Save merged/enriched outputs for transform.py to utilise
    (raw_dir / "merged_prs.json").write_text(json.dumps(merged_prs, indent=2), encoding="utf-8")
    (raw_dir / "enriched.json").write_text(json.dumps(enriched, indent=2), encoding="utf-8")

    print(f"[OK] Wrote merged_prs.json ({len(merged_prs)} PRs) and enriched.json to data/raw/")
